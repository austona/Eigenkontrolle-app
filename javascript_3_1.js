...
//Die ID des letzten geklickten Bildes wird gespeichert. Auch wird die Anzahl der Verbleibenden KÃ¼he in der Anzeige angepasst.
function rememberID(clicked_ID) {
	...
	if (clicked_ID == "LCS_High" || clicked_ID == "LCS_Low") {
		document.getElementsByClassName("remaining_Cows")[0].setAttribute("value", "Verbleibende KÃ¼he: " + remaining_Cows_Number);
		for (i = 1; i < numberPages; i++) {
			document.getElementsByClassName("remaining_Cows")[i].setAttribute("value", "Verbleibende KÃ¼he: " + remaining_Cows_Number);
		}
	}
	else
...
// Es wird geprÃ¼ft, ob gerade die letzte Kuh dran ist. Wenn ja, dann wird zur auswertung weitergeleitet, ansonsten geht es zurÃ¼ck an den anfang
...
//Der Counter muss wieder hochgestuft werden, wenn ein LCS Bild korrigiert wird. AuÃŸerdem wird eine Doppelkorrektur und eine korrektur ganz am Anfang verhindert.
function adjustCounter() {
	...
	if (curr_Id == "LCS_High" || curr_Id == "LCS_Low") {
		...
		for (i = 0; i < numberPages; i++) {
			document.getElementsByClassName("remaining_Cows")[i].setAttribute("value", "Verbleibende KÃ¼he: " + current_Number);
		}
	}
	else if (curr_Id == "karpalgelenk_Vorne_Weiter") {
		window.location.href = "#karpalgelenk_Vorne"
		$('#isDoubleCorrection').val(1);
		window.alert('Sie mÃ¼ssen zuerst eine Eingabe tÃ¤tigen, bevor Sie diese korrigieren kÃ¶nnen');
	}
...
// Sofern nicht zwei mal hintereinander auf zurÃ¼ck gedrÃ¼ckt wurde, wird man zur letzten Seite geleitet
...
// Alle Werte werden aus dem lokalen Speicher gelÃ¶scht 
function resetAll() {
	localStorage.clear();
	location.reload();
...
function createPDF() {
	...
	doc.text('Anzahl Ã¼berprÃ¼fter Tiere: ' + $('#anzahl_Gepruefter_Tiere').val(), 10, 10);

	doc.text('Gesundheit, Integument, Fortbewegung', 30, 30);
	var columnsGangbild = [{ title: "Bewegungsapparat", dataKey: "parameterBewegunsapperat" }, { title: "Anteil von KÃ¼hen [in %] mit: LCS >= 3", dataKey: "anteilGangbild" }, { title: "Bewertung", dataKey: "bewertungGangbild" }];
	...
	var columnsKopfhaltung = [{ title: "Fortbewegung", dataKey: "parameterFortbewegung" }, { title: "Anteil gehender KÃ¼he [in %] mit gerader Kopfhaltung", dataKey: "anteilGeradeKopfhaltung" }, { title: "Bewertung", dataKey: "bewertungKopfhaltung" }];
	...
	{ title: "Anteil von KÃ¼hen [in %] mit: kein Befund", dataKey: "anteilKeinBefund" },
	{ title: "Anteil von KÃ¼hen [in %] mit: haarlose Stellen", dataKey: "anteilHaarloseStellen" },
	{ title: "Anteil von KÃ¼hen [in %] mit: Krusten", dataKey: "anteilKrusten" },
	{ title: "Anteil von KÃ¼hen [in %] mit: Umfangsvermehrung", dataKey: "anteilUmfangsvermehrung" },
	{ title: "Anteil von KÃ¼hen [in %] mit: behandelte Wunden", dataKey: "anteilBehandelteWunden" },
	{ title: "Anteil von KÃ¼hen [in %] mit: Abweichungen insgesamt", dataKey: "anteilAbweichungenInsgesamt" }];
	rowsTechnopathien = [{
	...
	{
		"bereich": "RÃ¼cken", "anteilKeinBefund": $('#ruecken_Kein_Befund_Percentage').val(), "anteilHaarloseStellen": $('#ruecken_Haarlose_Stellen_Percentage').val(),
	...
	{ title: "Anteil an KÃ¼hen [in %]", dataKey: "anteilEutergesundheit" }, { title: "Bewertung", dataKey: "bewertungEutergesundheit" }];
	var rowsEutergesundheit = [{ "parameterEutergesundheit": "Eutergesunde KÃ¼he", "anteilEutergesundheit": $('#anteil_Eutergesunde_Kuehe').val(), "bewertungEutergesundheit": "" },
	...
	var columnsFuetterung = [{ title: "KÃ¶rperkondition", dataKey: "parameterFÃ¼tterung" },
	{ title: "Anteil von KÃ¼hen [in %] mit: BCS < 2", dataKey: "anteilMager" },
	{ title: "Anteil von KÃ¼hen [in %] mit: 2 >= BCS <= 4.5", dataKey: "anteilGut" },
	{ title: "Anteil von KÃ¼hen [in %] mit: BCS > 4.5", dataKey: "anteilFett" },
	{ title: "Bewertung", dataKey: "bewertungFÃ¼tterung" }
	];
	var rowsFuetterung = [{
		"parameterFÃ¼tterung": "", "anteilMager": $('#BCS_Low_Percentage').val(),
		"anteilGut": $('#BCS_Good_Percentage').val(), "anteilFett": $('#BCS_High_Percentage').val(), "bewertungFÃ¼tterung": ""
	}];
	doc.autoTable(columnsFuetterung, rowsFuetterung, {
		theme: 'striped',
		startY: 104,
		columnStyles: { "bewertungFÃ¼tterung": { fillColor: [rFuetterung, gFuetterung, bFuetterung] } },
	});
	...
	var rowsLiegeverhalten = [{ "parameterLiegeverhalten": "Liegeboxennutzung" }, {
		"merkmalLiegeverhalten": "Cow Comfort Quotient (Anteil an KÃ¼hen [in %] korrekt in der Liegebox liegender KÃ¼he, von allen die Kontakt zur Box haben)",
		"bewertungLiegeverhalten": $('#CCQ').val()
	}, { "merkmalLiegeverhalten": "Anteil von KÃ¼hen [in %] die mindestens teilweise auÃŸerhalb liegen", "bewertungLiegeverhalten": $('#partially_Outside').val() },
	{ "parameterLiegeverhalten": "Liegeposition" }, { "merkmalLiegeverhalten": "natÃ¼rliche Liegeposition", "bewertungLiegeverhalten": $('#naturally_Laying').val() }]
